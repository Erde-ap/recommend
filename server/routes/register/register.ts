...
function saveurl(req, res, onetime_Url){
    ...
    let hashpass = getPhash(req.body.password);
        ...
        if(account){
            if(account.regest < getDate() && account.ac_st ==false){
                Users.remove({ _id:account._id }, (err)=> {
                    ...
                    let error = {status:'認証の期限が切れました再登録してください。'};
                    res.send(error);
                });
...
function hadSendmailError(err, req, res, resp) {
    let error ={status: 4, err:err}
    res.send(error);
}

function hadDbError(err, req, res) {
    let error ={status: 6 , err:err}
    res.send(error);
}

function hadRateoverError(err, req, res) {
    let error ={status: 13, err:err}
    res.send(error);
}
